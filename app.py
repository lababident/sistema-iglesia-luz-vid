...
def generar_pdf_informes(df_agrupado, f_ini, f_fin, t_ing, t_diezmo, apostol, presbiterio):
    ...
    pdf.cell(190, 10, txt="Iglesia Cristiana Luz y Vida", ln=True, align='C')
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 8, txt="REPORTE GENERAL DE INGRESOS Y APORTES", ln=True, align='C')
    pdf.set_font("Arial", '', 10)
    pdf.cell(190, 7, txt=f"Desde: {f_ini} hasta: {f_fin}", ln=True, align='C')
    pdf.ln(5)
    ...
    pdf.cell(95, 8, f"TOTAL DIEZMOS (16 ITEMS): {t_diezmo:,.2f} Bs", 1, 1, 'C', True)
    pdf.cell(95, 8, f"APORTE APOSTOL (10%): {apostol:,.2f} Bs", 1, 0, 'C', True)
    ...
    for _, row in df_agrupado.iterrows():
        red_nombre = str(row['Red']).encode('latin-1', 'replace').decode('latin-1')
        pdf.cell(90, 7, red_nombre, 1, 0, 'L')
        pdf.cell(50, 7, f"{row['Total_Bs']:,.2f}", 1, 0, 'R')
        pdf.cell(50, 7, f"{row['Diezmo_10']:,.2f}", 1, 1, 'R')
...
if login():
    ...
    if menu == "🏠 INICIO":
        ...
        f_fin = c2.date_input("Fecha Hasta", date.today())
        
        redes_seleccionadas = st.multiselect("Filtrar por Red / Origen (Por defecto se calculan todas)", options=REDES, default=REDES)
        ...
        try:
            ...
            presbiterio_monto = df_16[df_16['Red'] != "Red de Zabulom"]['Diezmo_10'].sum()
                        ...
                        if st.button(f"💾 Guardar Movimiento en {moneda}", type="primary"):
                            if desc_div:
                                ...
                                try:
                                    ...
                                    st.error("Espera unos segundos. Límite de Google excedido.")
                            else:
